    ...
    //userを見つけても、DBの"users"にはpermissionsの情報までは書かれていないためpermissions(権限)を復元する必要がある。
      .then((user) => {
        ...
        //privilegesコレクション(テーブル)を見に行って
          ...
          //ユーザーが持っているrole_codeが"privileges"コレクションに格納されているrole_codeと一致するかの確認
            .findOne({role_code: user.role_code})
            .then((privilege) => {
...
passport.use("local-strategy", new LocalStrategy({
  ...
  MongoClient.connect(CONNECTION_URL, OPTIONS, (error, client) => {
    ...
    db.collection("users").findOne({
      //"username"欄に入力されたメールアドレスとDBに格納されているemailが一致するものを探す
      email: username,
      ...
      //ユーザーが正しく取得できているかの判定
      if (user) {
        //※初回ログイン時、受け取りメールを介さずログインしようとした場合のエラー
        if (user.status != "Active") {
          done(null, false, req.flash("message", "認証用のURLを記載したメールを送信しておりますので、記載されたURLにアクセスし本登録を完了してください。"));
        } else {
        //正しく取得できていればログインへ
